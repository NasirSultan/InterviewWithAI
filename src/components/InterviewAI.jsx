...
export default function ShortInterviewQA() {
  ...
  const handleSubmit = async () => {
    ...
    try {
 const prompt = `You are a skilled technical interviewer and educator.

Generate a short JSON object based on the topic: "${topic}".

Guidelines:

- Limit content to science and technology topics (e.g., programming, engineering, AI, physics, etc.).
- If the topic is outside of this domain, respond with:
  {
    "note": "This topic is beyond the science/technology scope. Please refer to trusted documentation or educational sources for more information."
  }

If the topic is valid, return the following structure:

{
  "title": "A clear, one-line topic title.",
  "explanation": "A brief explanation (2–3 short lines) written in simple, plain English.",
  "questions": [
    {
      "q": "A direct, clear technical question.",
      "a": "A concise answer using plain English. If explaining code, include a 10-line code block. If the answer requires steps or guidance, format with hyphens (-) for bullets and bold subheadings using **like this**."
    },
    ... (5 total questions)
  ]
}

Return only the pure JSON object—no additional comments or text.`;
  ...
  return (
   <div
  className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 min-h-screen"
  onMouseUp={() => {
...
>
  ...
  {historyChunks.length > 0 && (
    <>
            ...
            {chunk.explanation && (
              <ReactMarkdown
                components={{
                  ...
                  strong: ({ children }) => (
                    <strong className="text-indigo-700">{children}</strong>
                  ),
                }}
              >
                {chunk.explanation}
                        ...
                        code({ inline, className, children }) {
                          ...
                          return inline ? (
                            ...
                            <pre className="bg-gray-900 text-white p-4 rounded overflow-x-auto text-sm my-2">
                              <code
                                className={`language-${language} font-mono whitespace-pre-wrap`}
                              >
                                {children}
                              </code>
                            </pre>
      ...
      {/* Follow-up input */}
      <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-[94%] sm:w-[90%] md:w-[80%] lg:w-[66%] bg-white p-4 rounded-xl z-40 shadow-lg">
      ...
      {/* Action buttons */}
     <div className="fixed bottom-20 right-4 flex flex-col gap-3 items-end z-50">
  <button
    onClick={resetAll}
    title="Reset Memory"
  ...
  <button
    ...
    } text-white w-12 h-12 flex items-center justify-center rounded-full shadow hover:opacity-90`}
  >
